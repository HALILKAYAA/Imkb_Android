...
public class HisseEndeks extends Fragment implements OnItemClickListener {
    private ApiService apiService;
    private ArrayList<Stocks> imkbList;
    public  ArrayList<Stocks> imkbListnew;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_hisse_endeks, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        imkbList = new ArrayList<>();
        imkbListnew = new ArrayList<>();
        recyclerView = view.findViewById(R.id.recylerHisse);
    ...
    private void setAdapter(ArrayList<Stocks> imkbList) {
        adapter = new ImkbAdapter(getActivity(), imkbList);
    ...
    private void getListService(final HandshakeModel model) {
        ...
        try {
            String ciphertextString = Cryption.encrypt("all".getBytes(),model.getAesKey().getBytes(),model.getAesIV().getBytes());
            ListBody body1 = new ListBody();
            body1.setPeriod(ciphertextString);
            String authsds = model.getAuthorization();
    ...
    public void onItemClick(int position) {
        Intent intent = new Intent(getActivity(), DetailScreenActivity.class);
            intent.putExtra("id",imkbList.get(position).getId());
