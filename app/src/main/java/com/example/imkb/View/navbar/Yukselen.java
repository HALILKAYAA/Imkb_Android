...
public class Yukselen extends Fragment implements OnItemClickListener {
    private ApiService apiService;
    private ArrayList<Stocks> yukselenList;
    public  ArrayList<Stocks> yukselenListnew;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_yukselen, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        yukselenList = new ArrayList<>();
        yukselenListnew = new ArrayList<>();
        recyclerView = view.findViewById(R.id.recylerYukselen);
    ...
    private void setAdapter(ArrayList<Stocks> yukselenList) {
        adapter = new ImkbAdapter(getActivity(), yukselenList);
    ...
    private void getListService(final HandshakeModel model) {
        ...
        try {
            String ciphertextString = Cryption.encrypt("increasing".getBytes(),model.getAesKey().getBytes(),model.getAesIV().getBytes());
                ...
                public void onResponse(@NonNull Call<ListModel> call, @NonNull Response<ListModel> response) {
                    if (response.isSuccessful()) {
                        if (response.body() != null) {
                            ...
                            yukselenList = listModel.getStocks();
                            for (Stocks stocksItem : yukselenList){
                                try {
                                    stocksItem.setSymbol(Cryption.decrypt(stocksItem.getSymbol().getBytes(),model.getAesKey().getBytes(),model.getAesIV().getBytes()));
                                    yukselenListnew.add(stocksItem);
                                    setAdapter(yukselenListnew);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
    ...
    public void onItemClick(int position) {
        ...
        intent.putExtra("id",yukselenList.get(position).getId());
