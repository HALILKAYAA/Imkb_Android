...
public class Hacim30 extends Fragment implements OnItemClickListener {
    private ApiService apiService;
    private ArrayList<Stocks> hacimOtuzList;
    public  ArrayList<Stocks> hacimOtuzListnew;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_hacim30, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        hacimOtuzList = new ArrayList<>();
        hacimOtuzListnew = new ArrayList<>();
        recyclerView = view.findViewById(R.id.recyclerHacim30);
    ...
    private void setAdapter(ArrayList<Stocks> hacimOtuzList) {
        adapter = new ImkbAdapter(getActivity(), hacimOtuzList);
    ...
    private void getListService(final HandshakeModel model) {
        ...
        try {
            String ciphertextString = Cryption.encrypt("volume30".getBytes(),model.getAesKey().getBytes(),model.getAesIV().getBytes());
                ...
                public void onResponse(@NonNull Call<ListModel> call, @NonNull Response<ListModel> response) {
                    if (response.isSuccessful()) {
                        if (response.body() != null) {
                            ...
                            hacimOtuzList = listModel.getStocks();
                            for (Stocks stocksItem : hacimOtuzList){
                                try {
                                    stocksItem.setSymbol(Cryption.decrypt(stocksItem.getSymbol().getBytes(),model.getAesKey().getBytes(),model.getAesIV().getBytes()));
                                    hacimOtuzListnew.add(stocksItem);
                                    setAdapter(hacimOtuzListnew);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
    ...
    public void onItemClick(int position) {
        ...
        intent.putExtra("id",hacimOtuzList.get(position).getId());
