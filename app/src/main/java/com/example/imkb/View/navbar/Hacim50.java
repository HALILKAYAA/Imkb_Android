...
public class Hacim50 extends Fragment implements OnItemClickListener {
    private ApiService apiService;
    private ArrayList<Stocks> hacimElliList;
    public  ArrayList<Stocks> hacimElliListnew;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_hacim50, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        hacimElliList = new ArrayList<>();
        hacimElliListnew = new ArrayList<>();
        recyclerView = view.findViewById(R.id.recylerHacim50);
    ...
    private void setAdapter(ArrayList<Stocks> hacimElliList) {
        adapter = new ImkbAdapter(getActivity(), hacimElliList);
    ...
    private void getListService(final HandshakeModel model) {
        ...
        try {
            String ciphertextString = Cryption.encrypt("volume50".getBytes(),model.getAesKey().getBytes(),model.getAesIV().getBytes());
                ...
                public void onResponse(@NonNull Call<ListModel> call, @NonNull Response<ListModel> response) {
                    if (response.isSuccessful()) {
                        if (response.body() != null) {
                            ...
                            hacimElliList = listModel.getStocks();
                            for (Stocks stocksItem : hacimElliList){
                                try {
                                    stocksItem.setSymbol(Cryption.decrypt(stocksItem.getSymbol().getBytes(),model.getAesKey().getBytes(),model.getAesIV().getBytes()));
                                    hacimElliListnew.add(stocksItem);
                                    setAdapter(hacimElliListnew);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
    ...
    public void onItemClick(int position) {
        ...
        intent.putExtra("id",hacimElliList.get(position).getId());
